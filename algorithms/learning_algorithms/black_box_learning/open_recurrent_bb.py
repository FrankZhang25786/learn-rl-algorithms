...
def make_train(config):
    ...
    def train(meta_params, rng, grid_type=6):
        ...
        meta_opt = recurrent_open_gru(
            hidden_size=config["HSIZE_OPTIM"],
            gru_features=int(config["HSIZE_OPTIM"] / 2),
        )
...
if __name__ == "__main__":
    ...
    meta_opt = recurrent_open_gru(
        hidden_size=args.hsize, gru_features=int(args.hsize / 2)
    )
    ...
    for k in all_configs.keys():
        ...
        all_configs[k]["HSIZE_OPTIM"] = args.hsize
